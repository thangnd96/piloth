...
JSON
# routing has no CLI verb of its own: os-start runs it and surfaces the whole
# payload under asset_routing, so the contract the model reads is the assertion.
out=$(python3 "$G" os-start '{"task_id":"e19-routing","intent":"ui routing fixture","task_signal":"UI/component","affected_paths":["src/components/x.tsx"]}')
...
out=$(python3 "$G" os-start '{"task_id":"e19-deploy","intent":"deploy routing fixture","task_signal":"release/deploy","affected_paths":["package.json"]}')
grep -q '"load_policy": "approval-required"' <<< "$out"
grep -q '"asset": "package.json:scripts.deploy"' <<< "$out"
grep -q '"decision": "approval_required"' <<< "$out"
# An unroutable signal is rejected by the contract validator instead of by a
# standalone routing verb; the rejection still happens before any state is written.
out=$(python3 "$G" os-start '{"task_id":"e19-bad","intent":"x","task_signal":"unknown","affected_paths":["a.txt"]}')
...
grep -q 'guard mode os-close' <<< "$out"
grep -q 'guard mode receipt-seal' <<< "$out"
...
out=$(printf '%s' '{
  ...
  "quality_gates": {
    "reuse_non_duplication": {
      "result": "PASS",
      "evidence": "existing helper candidate is present"
    }
  },
...
# corrupt JSONL must fail the doctor: receipt-seals is the remaining ledger
mkdir -p pilothOS/memory/state
printf 'not-json\n' > pilothOS/memory/state/receipt-seals.jsonl
out=$(python3 "$G" state-doctor)
grep -q '"result": "state_doctor_failed"' <<< "$out"
grep -q 'receipt seals jsonl' <<< "$out"
rm -f pilothOS/memory/state/receipt-seals.jsonl
