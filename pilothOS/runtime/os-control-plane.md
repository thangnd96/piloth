...
contract snapshot in the control-plane repo. `mode` is `standard` unless the
request asks for `strict`; strict adds gates and does not change context
loading. It writes `target-snapshot.json`. For git targets, the snapshot records
...
(`code|ui|design_tokens|docs|release|generic`), `mode` (`standard|strict`),
`budget`, `adapter`, `adapter_capabilities`, `specialists`, `work_packages` and
`user_overrides`. See `runtime/evidence-router.md` for the canonical route
contract and rollout modes.
...
## Mode And Cost Ledger

Two modes, chosen explicitly:

- `standard` (default) for normal code, docs and controlled-target work;
- `strict` for release/deploy, full design-token coverage, broad scope or
  absolute-claim risk.

Both load the same context; `strict` only adds gates. There is no adaptive
resolver: an earlier build had `lean` and `micro` tiers plus a heuristic that
picked between them, but across 32 real runs the heuristic chose `standard` 25
times and `strict` 7 times and never once selected either lean tier, so the
tiers and the resolver were removed rather than kept as untested branches.
...
(`spent_usd`/`remaining_usd`/`over_budget`) in `os-status` — it never blocks
`os-close`. See `runtime/energy-token-policy.md`.
...
`os-close` runs the **safe subset** (os-run artifacts, never
